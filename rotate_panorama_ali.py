...
import requests
import torch
from alibabacloud_credentials.providers import EnvironmentVariableCredentialsProvider
...
from alibabacloud_objectdet20191230.client import Client as objectdet20191230Client
...
# model.set_classes(['manhole cover', 'dog', 'person', 'trash can'])
...
config.endpoint = f'objectdet.cn-shanghai.aliyuncs.com'
client = objectdet20191230Client(config)

# res = requests.get(image_url)
# img = cv2.imdecode(np.fromstring(res.content, dtype=npfuint8), cv2.IMREAD_COLOR)
...
for idx_conf in confs:
    ...
    for img_idx, image_url in enumerate(images):
        ...
        for j in range(7):
            ...
            if not os.path.exists(os.path.join(dir, '阿里云ai-标注图')):
                os.mkdir(os.path.join(dir, '阿里云ai-标注图'))
            ...
            for idx, pers_image in enumerate(pers_imgs):
                success, img = cv2.imencode('.jpg', pers_image.pers_img)
                detect_object_request = objectdet_20191230_models.DetectObjectAdvanceRequest(io.BytesIO(img.tobytes()))
                runtime = util_models.RuntimeOptions()
                try:
                    det_result = client.detect_object_advance(detect_object_request, runtime)
                ...
                for element in det_result.body.data.elements:
                    cls_np = element.type
                    if cls_np not in name_dict.keys():
                        continue
                    box_np = element.boxes
                    conf_np = element.score
                ...
                cv2.imwrite(f'{dir}/阿里云ai-标注图/labeled_img_{idx}.jpg',
        ...
        cv2.imwrite(f'det_merged/{img_names[img_idx]}_aliyun_det_result.jpg', img)
